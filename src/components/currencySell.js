...
export default function CurrencySellComp() {
    const [unit, setUnit] = useState(0);
    const [holdingUnit, setHoldingUnit] = useState(0);
    ...
    const fetchPortfolio = async () => {
        try {
            ...
            if (data.success === false) throw Error(data.message);
            setCurrency(param)
            setHoldingUnit(data.portfolio[param])
        }
        catch (error) {
            console.log(error)
            var s = error.message || 'Something went wrong...Try again...';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // return
            if (holdingUnit < unit) {
                throw Error("Insufficient units in portfolio")
            }
            const { data } = await axios.post(`${API}/sell`,
                {
                    "currency": currency,
                    "amount": `${amount}`
                },
                { headers: { token: token } },);
            console.log("sell request queued", data);
            if (data.success == false) {
                console.log(data.message)
                throw Error(data.message)
            }
            toast.success(" Sell request queued ");
            setLoading(false);
            setUnit(0)
            navigate('/');
        }
    ...
    return (
        <div className="m-3" style={{ minHeight: '80vh' }}>
            {/* {user === null ? 'NULL' : 'else'} */}
            <h1 className="text-center square mb-3 mt-3">Sell Instrument</h1>
            <form
                onSubmit={handleSubmit}
            >
                <h2>Holding Unit: {holdingUnit}</h2>
